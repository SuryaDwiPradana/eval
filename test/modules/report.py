...
def main_report(config):
    ...
    for cat in tqdm(config.data,ascii=True,desc="Result", position=1):
        ...
        for y in config.data[cat]:
            for nim in tqdm(y,ascii=True,desc=cat, position=0):
                ...
                fill_report(all_recap,file_ref[nim],config.data[cat][0][nim][0]['nama'],nim,config.report['organization'],config.data[cat][0][nim][0]['jabatan'],config.report['period'])
                fill_report(solo_recap,file_ref[nim],config.data[cat][0][nim][0]['nama'],nim,config.report['organization'],config.data[cat][0][nim][0]['jabatan'],config.report['period'])
    ...
    all_recap.save(config.config['resources_path']+'/output/'+config.folder['dirname'][2]+"/Recap Laporan Evaluasi.xlsx")
    return
