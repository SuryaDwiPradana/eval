...
def main_report(config):
    try:
        ...
        file_ref = config.load_sheet(
            config.config['resources_path'] + '/output/' + config.folder['dirname'][1] + '/' + file_summary + '.xlsx',
            True)
        ...
        for cat in tqdm(config.data, ascii=True, desc="Result", position=1):
            ...
            for nim in tqdm(config.data[cat], ascii=True, desc=cat, position=0):
                ...
                solo_recap.save(config.config['resources_path'] + '/output/' + config.folder['dirname'][
                    2] + "/" + cat + '/' + nim + ".xlsx")
                all_recap.active = 0
        all_recap[all_recap.active.title].sheet_state = "hidden"
        all_recap.save(config.config['resources_path'] + '/output/' + config.folder['dirname'][
            2] + "/Evaluation Report Recap.xlsx")
